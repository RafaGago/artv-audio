...
private:
  ...
  // stubs for sliders
public:
  double get_slider_compensate()
  {
    // TODO: stub, add code for getting "compensate"
    // Range: min:-20.0, max:20.0, default: 0.0, step: 0.0005
    // Original line: slider5:compensate=0<-20,20,0.0005>Gain compensation
    return 0.;
  }
  // Snippet for parameter boilerplate in the authors C++ framework....
  /*
  float compensatep = 0.0;
  ...
  */
  //----------------------------------------------------------------------------
  double get_slider_gainsmoothing()
  {
    // TODO: stub, add code for getting "gainsmoothing"
    // Range: min:0.0, max:1.0, default: 0.0, step: 1e-05
    // Original line: slider8:GainSmoothing=0<0,1,0.00001>Gain smoothing
    return gainsmoothingp;
  }
  // Snippet for parameter boilerplate in the authors C++ framework....
  float gainsmoothingp = 0.0;
  ...
  //----------------------------------------------------------------------------
  double get_slider_mode()
  {
    // TODO: stub, add code for getting "mode"
    // Range: min:0.0, max:1.0, default: 0.0, step: 1.0
    // Original line: slider6:Mode=0<0,1,1>Peak/Squared
    return modep;
  }
  // Snippet for parameter boilerplate in the authors C++ framework....
  float modep = 0.0;
...
//----------------------------------------------------------------------------
  ...
  double get_slider_oversampling()
  {
    ...
    // Original line: slider7:Oversampling=1<1,4,1>Oversampling
    return oversamplingp;
  }
  // Snippet for parameter boilerplate in the authors C++ framework....
  float oversamplingp = 1.0;
  ...
  {
    // TODO: stub, add code for getting "sattack"
    // Range: min:0.0, max:1.0, default: 0.5, step: 1e-09
    // Original line: slider1:sAttack=0.5<0,1,.000000001>Attack
    return sattackp;
  }
  // Snippet for parameter boilerplate in the authors C++ framework....
  ...
  //----------------------------------------------------------------------------
  double get_slider_sdecay()
  {
    // TODO: stub, add code for getting "sdecay"
    // Range: min:0.0, max:1.0, default: 0.5, step: 1e-09
    // Original line: slider2:sDecay=0.5<0,1,.000000001>Decay
    return sdecayp;
  }
  // Snippet for parameter boilerplate in the authors C++ framework....
  ...
  double get_slider_strength()
  {
    ...
    // Range: min:-1.0, max:1.0, default: 0.0, step: 1e-09
    // Original line: slider3:strength=0<-1,1,.000000001>Reduce / Enhance Attack
    return strengthp;
  }
  ...
  //----------------------------------------------------------------------------
  double get_slider_strength2()
  {
    // TODO: stub, add code for getting "strength2"
    // Range: min:-1.0, max:1.0, default: 0.0, step: 1e-09
    // Original line: slider4:strength2=0<-1,1,.000000001>Reduce / Enhance Decay
    return strength2p;
  }
  float strength2p = 0.0;
...
private:
  ...
  double envdesireddecay$at;
  double envdesireddecay$rt;
  double envdesireddecay$state;
  double envfollower$at;
  double envfollower$rt;
  double envfollower$state;
  double envfollowerout$at;
  double envfollowerout$rt;
  double envfollowerout$state;
  double envout;
  double inl;
  double inputgain;
  double inr;
  ...
  void init_block_variables()
  {
    ...
    envdesireddecay$at     = 0;
    envdesireddecay$rt     = 0;
    envdesireddecay$state  = 0;
    envfollower$at         = 0;
    envfollower$rt         = 0;
    envfollower$state      = 0;
    envfollowerout$at      = 0;
    envfollowerout$rt      = 0;
    envfollowerout$state   = 0;
    envout                 = 0;
    inl                    = 0;
    inputgain              = 0;
    inr                    = 0;
...
public:
  ...
  void process_block_replacing (std::array<T*, 2> chnls, uint samples)
  {
    ...
    attack = std::exp (alpha * get_slider_sattack() + beta) - 1.;
    ...
    curfirdelay = init$getfirdelay (get_slider_oversampling());
    ...
    for (int $$i = 0, $$end = samples; $$i < $$end; ++$$i) {
      ...
      if (eel2_eq (get_slider_oversampling(), 1.)) {
        ...
        f = 0.;
        for (int $$i2   = 0,
                 $$end2 = std::max (0, (int) (get_slider_oversampling()));
             $$i2 < $$end2;
             ++$$i2) {
          f += 1.;
          inl
      ...
      init$updatebuffer (
        dbgain, gainbuf$scopeptr, gainbuf$scopebuffermax, gainbuf$scopebuffer);
      init$updatebuffer (
        envout, outbuf$scopeptr, outbuf$scopebuffermax, outbuf$scopebuffer);
      ;
#endif
    }
  }
...
//----------------------------------------------------------------------------
    ...
    double& x)
  {
    ...
    return [&] {
      if (scrolls) {
        ...
        for (int $$i = 0,
                 $$end
             = std::max (0, (int) ((scopebuffermax - scopeptr) / skipfactor));
             $$i < $$end;
             ++$$i) {
          yy = ybase
            - scale * std::max (floorlevel, mempool[(size_t) (lptr)] - dbmax);
          if (fill) {
            gfx_triangle (
              xx - dx,
              y + h,
              ...
              y + h,
              xx,
              yy) /*TODO: unknown call */;
          }
          gfx_lineto (xx, yy) /*TODO: unknown call */;
          ly = yy;
          lptr += skipfactor;
          xx += dx;
        }
        ...
        return [&] {
          ...
          for (int $$i = 0,
                   $$end
               = std::max (0, (int) ((scopeptr - scopebuffer) / skipfactor));
               $$i < $$end;
               ++$$i) {
            yy = ybase
              - scale * std::max (floorlevel, mempool[(size_t) (lptr)] - dbmax);
        ...
        return [&] {
          ...
          for (int $$i   = 0,
                   $$end = std::max (
                     0, (int) ((scopebuffermax - scopebuffer) / skipfactor));
               $$i < $$end;
               ++$$i) {
            yy = ybase
              - scale * std::max (floorlevel, mempool[(size_t) (lptr)] - dbmax);
  ...
  //----------------------------------------------------------------------------
  double init$draw_compressor_window (
    double  floorlevel,
    double  compression,
    double  gradblittarget,
    double& lw,
    double& w,
    double& lh,
    double& h,
    ...
    double& meterspace)
  {
    ...
    double olddest  = 0.;
    gfx_mode        = 0.;
    gfx_set (1., 1., 1., 1.) /*TODO: unknown call */;
    if ((eel2_ne (lw, w) || eel2_ne (lh, h))) {
      lw      = w;
      lh      = h;
      olddest = gfx_dest;
      gfx_setimgdim (gradblittarget, w, h) /*TODO: unknown call */;
      gfx_dest = gradblittarget;
      gfx_set (0., 0., 0., 1.) /*TODO: unknown call */;
      gfx_rect (x, y, w, h) /*TODO: unknown call */;
      gfx_gradrect (
        x,
        y,
        w,
      ...
      gfx_gradrect (
        ...
        .0001,
        .01,
        .0004) /*TODO: unknown call */;
      gfx_dest = olddest;
    }
    gfx_y = 0.;
    gfx_x = gfx_y;
    gfx_blit (gradblittarget, 1., 0.) /*TODO: unknown call */;
    stepsize = 3.;
    cstep    = dbmax;
    dy       = stepsize * h / floorlevel;
    gfx_set (1., .3, .3, .6) /*TODO: unknown call */;
    steps = std::ceil (h * (compression / floorlevel) / 2.);
    yc    = y;
    xs    = x + (w - meterspace) + 4.;
    xe    = x + w - 4.;
    for (int $$i = 0, $$end = std::max (0, (int) (-steps)); $$i < $$end;
         ++$$i) {
      gfx_line (xs, std::ceil (yc), xe, std::ceil (yc)) /*TODO: unknown call */;
      yc += 2.;
    }
    steps = std::ceil (-floorlevel / stepsize);
    yc    = y;
    gfx_setfont (1., 0.; /* jsfx2cpp strings unsupported: was: "Arial" */,
                         16.) /*TODO: unknown call */;
    return [&] {
      double $$loop_ret_0 = {};
      for (int $$i = 0, $$end = std::max (0, (int) (steps)); $$i < $$end;
           ++$$i) {
        gfx_set (1., 1., 1., .7) /*TODO: unknown call */;
        if (cstep > 0.) {
          sprintf (0., 0.; /* jsfx2cpp strings unsupported: was: "%2d" */,
                           cstep) /*TODO: unknown call */;
        }
        else {
          sprintf (0., 0.; /* jsfx2cpp strings unsupported: was: "-%2d" */,
                           cstep) /*TODO: unknown call */;
        }
        gfx_measurestr (0., ww, hh) /*TODO: unknown call */;
        gfx_x = x + w - .5 * meterspace - .5 * ww;
        gfx_y = yc - .5 * hh - 1.;
        gfx_drawstr (0.) /*TODO: unknown call */;
        gfx_set (1., 1., 1., .2) /*TODO: unknown call */;
        gfx_line (
          x,
          std::ceil (yc),
          x + w - .8 * meterspace,
          std::ceil (yc)) /*TODO: unknown call */;
        gfx_line (
          x + w - .2 * meterspace,
          std::ceil (yc),
          x + w,
...
#endif
  //----------------------------------------------------------------------------
  double init$eval_follower (double x, double& state, double& rt, double& at)
  {
...
//----------------------------------------------------------------------------
  ...
  double block$processsample()
  {
    (inputgain = 20.
       * log10 (std::max (
         .001, .5 * (inl * inl + inr * inr))) /*TODO: unknown call */);
    if (eel2_eq (get_slider_mode(), 0.)) {
      (inputgain = 20.
         * log10 (std::max (
           .001,
           .5 * (std::abs (inl) + std::abs (inr)))) /*TODO: unknown call */);
    }
    else {
      inputgain;
    }
    cenv = init$eval_follower (
      inputgain, envfollower$state, envfollower$rt, envfollower$at);
    ctargetatk = init$eval_follower (
      inputgain,
      envdesiredattack$state,
      envdesiredattack$rt,
      envdesiredattack$at);
    ctargetdecay = init$eval_follower (
      inputgain, envdesireddecay$state, envdesireddecay$rt, envdesireddecay$at);
    ldiffatk  = get_slider_strength() * (ctargetatk - cenv);
    ldiffrel  = get_slider_strength2() * (ctargetdecay - cenv);
    dbgaincur = -ldiffatk + ldiffrel + get_slider_compensate();
    dbgain    = alpha_gain * dbgain + (1. - alpha_gain) * dbgaincur;
    lingain   = eel2_pow (10., (0.05 * dbgain));
    outl      = inl * lingain;
    outr      = inr * lingain;
    (movout
     = 20. * std::log10 (std::max (.001, .5 * (outl * outl + outr * outr))));
    if (eel2_eq (get_slider_mode(), 0.)) {
      (movout = 20.
         * std::log10 (
                  std::max (.001, .5 * (std::abs (outl) + std::abs (outr)))));
    }
    else {
      movout;
    }
    envout = init$eval_follower (
      movout, envfollowerout$state, envfollowerout$rt, envfollowerout$at);
    return envout;
  }
}; /* transience */
